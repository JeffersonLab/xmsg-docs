...
A *proxy* must be running in order to pass messages betwen actors.
...
The **default proxy** address should be used for long-running publication
or subscription of messages.
...
![](img/raw-message "raw-message"){.zmqmsg}
...
```
The *mime-type* will be used to check if the message contains data of the
expected type.
Primitives and arrays of primitives should have been serialized as
*protobuffer* format.
...
In order to publish or subscribe messages, a connection to an xMsg **proxy**
must be obtained.
Connections are managed by the xMsg actor, that keeps an internal **connection pool**
to cache and reuse connections.
...
![](img/ctrl-pub-req "PUB control message"){.zmqmsg}
...
![](img/ctrl-pub-ack "PUB control response"){.zmqmsg}
...
xMsg provides two wrappers scripts to launch the **proxy** and the
**registrar** processes.
